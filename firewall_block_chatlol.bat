...
if %errorlevel% NEQ 0 (
  ...
  powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Execute as admin', 'Runas Admin', 'ok', [System.Windows.Forms.MessageBoxIcon]::Error);}"
  goto end
)
...
FOR /f "delims=[] tokens=2  " %%a IN ('ping -n 1 %server% -6 ^|findstr "["') DO (
 ...
 IF %check1%==1 (
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Chat OFF', 'Rules has beenAdd', 'ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
) else (
powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Chat ON', 'Rules has been Removed', 'ok', [System.Windows.Forms.MessageBoxIcon]::Information);}"
)
